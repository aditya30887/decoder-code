function data_out = modi_filter_dblpass(data_in, method, varargin)
% 
% modified from Ian's c3d_load_v3 package
% 
% syntax:
% DATA_OUT = C3D_FILTER_DBLPASS(DATA_IN, METHOD, 'fc', FC, 'fs', FS)
% if FS is not specified, 1000 Hz is taken as default.
% 
...
% 
if strmatch(method, 'standard', 'exact')
    data_out = double_pass_filter_standard(data_in, n_reflect, B, A, zibasis);
else
    data_out = double_pass_filter_enhanced(data_in, n_reflect, B, A, zibasis);
end
end
