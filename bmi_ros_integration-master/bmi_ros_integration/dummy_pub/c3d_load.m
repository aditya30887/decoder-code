...
%C3D_LOAD 
%   Load a .c3d or set of .c3d files and format into simplified structure.
%
...
% 		c3dstruct(x).Left_HandY = c3d.Hand.LeftY;
		...
		if ~isempty(to_keep)
			...
			for i = 1:length(fnames)
				...
				%expressions 
				if isempty( cell2mat( regexp(upper(fnames{i}), upper(to_keep)) ) )
					c3dstruct = rmfield(c3dstruct, fnames{i});
				end
			end
		...
		if ~isempty(to_ignore)
			...
			for i = 1:length(fnames)
				...
				if ~isempty( cell2mat( regexp(upper(fnames{i}), upper(to_ignore)) ) )
					c3dstruct = rmfield(c3dstruct, fnames{i});
				end
			end
